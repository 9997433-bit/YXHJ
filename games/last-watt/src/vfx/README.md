...
| `selfcheck.ts` | 无头自检，19 项断言，不需要 WebGL |
| `demo/` | VFX Gym（纯粒子）、引擎接线试验台、冰碎可读性像素探针 |
...
`uCull` 只能整层进/不进，而冰碎需要**同一层里**辉光让位给碎片。Round 2 的冰碎
就栽在这儿：闪光、辉光、碎片一起全额进 Bloom，泛光把碎片吃成一坨白饼
（探针实测 +100ms 有 28% 的像素糊到近白，碎片轮廓的边缘能量只剩 0.05）。

所以逐粒子多一个 `aBloom` 属性（0..1），片元里 `rgb *= mix(1.0, vBloom, uMaskPass)`：
**beauty pass 该多亮还多亮，只削它往 Bloom 输入里投的那份能量**。
调暗辉光和调低它的泛光贡献从此是两件事。冰碎据此让辉光限额、碎片全额，
再用 `renderOrder`（加色 10 / 混色 11）保证碎片画在辉光之上。

这条链子有专门的回归闸门：自检里「冰碎：辉光让位给碎片」，
以及 `demo/readability.probe.mjs` 的 A/B 像素探针（见下）。
...
```bash
npm run selfcheck       # 无头自检：19 项断言，不需要 GPU，可进 CI
npm run probe:shatter   # 冰碎可读性 A/B 像素探针，需要 Chrome（SwiftShader 即可）
...
**可读性探针**（`demo/readability.probe.mjs`）补的是自检的盲区：
「糊白」是像素层面的事，断言 `aBloom` 写对了不等于画面能看。
它在无头 Chrome 里把 R2 与当前调校各跑一遍同一发冰碎，在 +17/100/200/350ms
四个时刻读画布像素，量六个指标（糊白占比、边缘能量、平均亮度、冰像素占比、
冰像素蓝调、冰像素边缘），A/B 对比后给判定。`--png` 另存八张对照图。

判定不能只有「别糊白」——把粒子全调没也能让糊白归零。所以另外三条反向兜底：
冰像素占比逐帧 ≥3%（没把冰调没）、冰像素蓝调 >0.06（还是冰不是灰）、
边缘能量必须**上升**（碎片真长出了轮廓）。当前读数：+100ms 糊白 28.21% → 0.08%，
同帧冰像素边缘能量 0.050 → 0.292。
...
- 音效不在本模块。`src/audio` 与本模块的 `combatBridge.ts` 订阅**同一条**战斗信号，
  靠同步派发保证同帧；两边都不认识对方。
