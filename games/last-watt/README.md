...
**默认跑的就是 `data/waves.map1.json` 的真实解锁表**：第 1 波只有铆钉机枪，
焦油第 2 波到，冷凝 / 破碎锤 / 发电机第 3 波到。建造条上锁着的格子会写明「第 N 波」，
图纸落地的那一帧老周会在对讲框里报一声。切片不再默认解锁全部蓝图——
那是玩家永远见不到的节奏。
...
| 右键 / `Esc` | 取消当前武装或选中 |
| 空格 | 开波。波之间无限等待，所以每次开波都算提前开波，拿 +10% 赏金——与顶栏按钮同一条命令 |
| `D` / `B` | 挖沟 / 搭桥（各有配额，合法格会高亮） |
| `Q` | 主控过载 |
| `P` | 暂停 / 继续（时间尺压到 0，画面与 HUD 照常刷新） |
| `R` | 本局结束后重开 |
| `Z` / 滚轮 | 两档缩放 |
| `H` | 收起右上角的调试计数条 |

调试键（**会改变节奏，按下即在计数条的 `dev aids` 一行留痕**）：

| 键 | 作用 |
|---|---|
| `G` | +400 金。只补钱包，跳不过解锁表 |
| `U` | 解除解锁表，五个 M1 蓝图全开；再按一次恢复 |

`Game.diagnostics().devAids` 与右上角计数条同源。**这一行不为空的跑测，不能当默认节奏或性能证据。**
...
1. `U` 解除解锁表，`G` 拿够钱（冷凝 130 + 破碎锤 120，开局 220 金买不下这一对）。
...
**不按调试键的走法**是教学本来的样子：第 1–2 波用两台机枪顶住，第 2 波清完时冷凝与破碎锤到货，
在波间摆好这一对，第 3 波的装甲卡车撞上去就是第一次冰碎。
（按默认赏金，清完第 2 波离 250 金还差几十块，第一对塔多半要等第 3 波打到一半才凑齐——
这是配表侧的数值缺口，不是接线层的开关问题。）
...
```js
__lastWattGame.game.diagnostics()   // fps / sim hz / 波次 / 金钱 / 电力 / 塔与敌人数 / 粒子 / 已播 VFX / 已解锁图纸 / devAids
__lastWattGame.game.session.commands.buildAt('condenser_jet', 5, 1)
__lastWattGame.game.session.commands.startWave({ early: true })
__lastWattGame.game.combat.bus.on('ice_shatter', console.log)
```

要在一波之内验完冰碎链的无头探针，用构造参数而不是热键：
`new Game({ container, unlockAll: true })` 与 `U` 是同一个开关，同样会记进 `devAids`。
...
│  │  ├─ runOverlay.ts #   暂停横幅与本局结束面板（HUD 信息架构之外的两个屏幕状态）
